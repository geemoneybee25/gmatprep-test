...
const ScoreChart: React.FC<{ 
  ...
  // GMAT boundaries
  const floor = isOverall ? 205 : 60;
  const ceiling = isOverall ? 805 : 90;
  
  // Calculate dynamic bounds to "zoom in" on the data
  const { min, max } = useMemo(() => {
    ...
    // If data is flat or range is tiny, provide a default window
    if (range < 10) {
      range = isOverall ? 100 : 5;
    }

    // Add padding to the range for visual comfort, but stay within GMAT limits
...
const ProgressPage: React.FC<ProgressPageProps> = ({ skills, stats, engagement, pastTests, onSelectSkill, initialTab }) => {
  ...
  const initialSectionScore = useMemo(() => {
    if (!baselineTest) return 60;
    if (activeDomain === Domain.QUANT) return baselineTest.mathScore;
    if (activeDomain === Domain.VERBAL) return baselineTest.readingScore;
    return baselineTest.diScore || 60;
  ...
  const projectedSectionScore = useMemo(() => {
    ...
    // GMAT subscore is 60-90, range is 30
    const calculated = 60 + Math.round(avgMastery * 30);
    return Math.max(calculated, initialSectionScore + 2);
  }, [filteredSkills, initialSectionScore]);

  const mathBaseline = baselineTest?.mathScore || 60;
  const readingBaseline = baselineTest?.readingScore || 60;
  const diBaseline = baselineTest?.diScore || 60;

  const mathAvgMastery = useMemo(() => {
    const s = skills.filter(sk => sk.domain === Domain.QUANT);
    return s.length > 0 ? s.reduce((acc, sk) => acc + sk.pMastery, 0) / s.length : 0;
  }, [skills]);

  const readingAvgMastery = useMemo(() => {
    const s = skills.filter(sk => sk.domain === Domain.VERBAL);
    return s.length > 0 ? s.reduce((acc, sk) => acc + sk.pMastery, 0) / s.length : 0;
  }, [skills]);

  const diAvgMastery = useMemo(() => {
    const s = skills.filter(sk => sk.domain === Domain.DI);
    return s.length > 0 ? s.reduce((acc, sk) => acc + sk.pMastery, 0) / s.length : 0;
  }, [skills]);

  // Formulas updated to GMAT 60-90 scale
  const mathPredicted = Math.max(60 + Math.round(mathAvgMastery * 30), mathBaseline + 2);
  const readingPredicted = Math.max(60 + Math.round(readingAvgMastery * 30), readingBaseline + 2);
  const diPredicted = Math.max(60 + Math.round(diAvgMastery * 30), diBaseline + 2);
  
  // Total score calculation (approximation based on mastery across all domains)
  const totalAvgMastery = useMemo(() => {
    return skills.length > 0 ? skills.reduce((acc, sk) => acc + sk.pMastery, 0) / skills.length : 0;
  }, [skills]);
  const overallPredictedScore = 205 + Math.round(totalAvgMastery * 600);
  
  const targetScoreOverall = Math.min(Math.max(overallPredictedScore + 60, 755), 805);
        ...
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-xl shadow-slate-100 flex flex-col justify-center overflow-hidden relative min-h-[200px]">
               <div>
                  <h3 className="text-slate-600 text-xs font-black uppercase tracking-widest mb-2">Predicted GMAT</h3>
              ...
              <div>
                ...
                <div className="mt-4 flex gap-1 h-8 items-end">
                   {scoreTrendOverall.map((s, i) => (
                     <div key={i} className="flex-1 bg-slate-900/10 rounded-t-sm" style={{ height: `${((s - 205) / 600) * 100}%` }} />
                   ))}
                </div>
              </div>
            </div>
             ...
             <div className="space-y-4">
                <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex items-center justify-between">
                   <div className="flex items-center gap-4">
                     ...
                     <div>
                       <h4 className="font-bold text-slate-800 text-sm">Quantitative</h4>
                       <p className="text-[10px] font-bold text-slate-600 uppercase">Projected: {mathPredicted}</p>
                     </div>
                   </div>
                   <div className="w-40 md:w-48">
                      <ScoreChart id="math-trend" data={[...pastTests.map(t => t.mathScore), mathPredicted]} color="#F59E0B" height={40} isOverall={false} />
                   </div>
                </div>
                <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex items-center justify-between">
                   <div className="flex items-center gap-4">
                     ...
                     <div>
                       <h4 className="font-bold text-slate-800 text-sm">Verbal</h4>
                       <p className="text-[10px] font-bold text-slate-600 uppercase">Projected: {readingPredicted}</p>
                     </div>
                   </div>
                   <div className="w-40 md:w-48">
                      <ScoreChart id="reading-trend" data={[...pastTests.map(t => t.readingScore), readingPredicted]} color="#3B82F6" height={40} isOverall={false} />
                   </div>
                </div>
                <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex items-center justify-between">
                   <div className="flex items-center gap-4">
                     ...
                     <div>
                       <h4 className="font-bold text-slate-800 text-sm">Data Insights</h4>
                       <p className="text-[10px] font-bold text-slate-600 uppercase">Projected: {diPredicted}</p>
                     </div>
                   </div>
                   <div className="w-40 md:w-48">
                      <ScoreChart id="di-trend" data={[...pastTests.map(t => t.diScore || 60), diPredicted]} color="#10B981" height={40} isOverall={false} />
                   </div>
                </div>
                <div className="bg-indigo-50 rounded-3xl p-6 border border-indigo-100">
                   <h4 className="text-indigo-900 font-black text-base mb-1">Strategy Insight</h4>
                   <p className="text-indigo-900 text-xs leading-relaxed">
                     Your current mastery trajectory suggests a strong upward trend. Focus on {Math.min(mathAvgMastery, readingAvgMastery, diAvgMastery) === mathAvgMastery ? 'Quantitative' : Math.min(mathAvgMastery, readingAvgMastery, diAvgMastery) === readingAvgMastery ? 'Verbal' : 'Data Insights'} to balance your profile.
                   </p>
                </div>
             </div>
          </div>
