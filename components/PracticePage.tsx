...
const PROGRESSIVE_DRILL: Problem[] = [
  {
    ...
    renderAsPlain: true,
    explanation: "Factor the quadratic: x² - 5x + 6 = (x - 2)(x - 3) = 0.\nSetting each factor to zero: x - 2 = 0 or x - 3 = 0.\nTherefore, x = 2 or x = 3.",
    feedback: "Excellent! You correctly factored the quadratic equation.",
    hints: {
      strategic: "Factor the quadratic expression into two binomials.",
      conceptual: "Look for two numbers that multiply to 6 and add to -5.",
      procedural: "The factors are (x - 2) and (x - 3). Set each equal to zero and solve.",
      bottomOut: "The solutions are x = 2 and x = 3."
    }
  },
  {
    id: 'p2',
    question: "A company's profit increased by 20% from Year 1 to Year 2, then decreased by 20% from Year 2 to Year 3. If the profit in Year 1 was $100,000, what was the profit in Year 3?",
    answer: "$96,000",
    options: ["$96,000", "$100,000", "$104,000", "$92,000"],
    renderAsPlain: true,
    explanation: "Year 2 profit: $100,000 × 1.20 = $120,000.\nYear 3 profit: $120,000 × 0.80 = $96,000.\nNote: A 20% increase followed by a 20% decrease does NOT return to the original value.",
    feedback: "Great work! You avoided the trap of assuming the changes cancel out.",
    hints: {
      strategic: "Calculate the actual values after each percentage change, not just the net effect.",
      conceptual: "Percentage changes are multiplicative, not additive. 20% of different bases gives different results.",
      procedural: "After +20%: $100k × 1.2 = $120k. After -20%: $120k × 0.8 = $96k.",
      bottomOut: "The Year 3 profit is $96,000."
    }
  },
  {
    id: 'p3',
    question: "A biologist argues that introducing Species X to the ecosystem will control the population of invasive Species Y. However, critics point out that in similar ecosystems, Species X also preys on native Species Z, which is already endangered.\n\nWhich of the following, if true, would most WEAKEN the biologist's argument?",
    answer: "Species X has driven Species Z to extinction in three other ecosystems where it was introduced",
    options: [
      "Species Y has no natural predators in the current ecosystem",
      "Species X has driven Species Z to extinction in three other ecosystems where it was introduced",
      "The population of Species Y has doubled in the past five years",
      "Species Z plays a crucial role in pollinating native plants"
    ],
    renderAsPlain: true,
    explanation: "The biologist's plan assumes the benefit (controlling Species Y) outweighs any costs. Evidence that Species X has actually caused extinction of Species Z elsewhere directly undermines the plan's viability by showing a severe, documented negative consequence.",
    feedback: "Excellent critical reasoning! You identified the evidence that most damages the proposal.",
    hints: {
      strategic: "Look for evidence that the proposed solution has already caused serious problems elsewhere.",
      conceptual: "A weakener shows that the plan's negative consequences outweigh its benefits.",
      procedural: "The argument is: introduce x to control Y. The weakener shows: introducing X has killed Z in other places.",
      bottomOut: "The answer about Species X driving Species Z to extinction most weakens the argument."
    }
  }
];

const WEEKS_CONFIG: Record<number, { title: string; focus: string; domain: Domain }> = {
  1: { title: "Quantitative Foundations", focus: "Arithmetic & Algebra Basics", domain: Domain.QUANT },
  2: { title: "Verbal Foundations", focus: "Critical Reasoning Intro", domain: Domain.VERBAL },
  3: { title: "Data Insights Basics", focus: "Data Sufficiency Fundamentals", domain: Domain.DI },
  4: { title: "Advanced Problem Solving", focus: "Word Problems & Applications", domain: Domain.QUANT },
  5: { title: "Reading Comprehension", focus: "Main Ideas & Inference", domain: Domain.VERBAL },
  6: { title: "Multi-Source Reasoning", focus: "Integrated Analysis", domain: Domain.DI },
  7: { title: "Geometry & Coordinate Plane", focus: "Spatial Reasoning", domain: Domain.QUANT },
  8: { title: "Argument Analysis", focus: "Strengthen, Weaken, Evaluate", domain: Domain.VERBAL },
  9: { title: "Mixed Strategy Review", focus: "Cross-Section Reasoning", domain: Domain.QUANT },
  10: { title: "Final Simulation Week", focus: "Endurance & Timing", domain: Domain.QUANT }
};
...
const PracticePage: React.FC<PracticePageProps> = ({ skills, onUpdateSkill, onNavigateToAnalytics, onSelectSkill, adjustment, engagement, onSessionComplete, queuedSkillId, isDemoMode = false, onSimulateDiagnostic }) => {
  ...
  const [sidebarDomain, setSidebarDomain] = useState<Domain>(Domain.QUANT);
  ...
  const generateDefaultPlan = (week: number) => {
    ...
    const theme = WEEKS_CONFIG[week] || WEEKS_CONFIG[1];
    ...
    return days.map((day, i) => {
      ...
      if (i === 0) {
        dayActivities.push({ id: `act-${week}-${i}-1`, type: 'Lesson', title: `${getTopic(0)} Mastery`, subtitle: 'Educational Unit', duration: '15m', questionCount: 5, skills: [getTopic(0)], completed: isWeek1 && isMonday });
        dayActivities.push({ id: `act-${week}-${i}-2`, type: 'Guided Practice', title: `Applying ${getTopic(0)}`, subtitle: 'Active Feedback', duration: '20m', questionCount: 10, skills: [getTopic(0)], completed: isWeek1 && isMonday });
      } else if (i === 1) {
        dayActivities.push({ id: `act-${week}-${i}-1`, type: 'Practice Drill', title: `Topic Simulation Drill`, subtitle: 'Blind Assessment', duration: '25m', questionCount: 15, skills: [getTopic(1)], completed: false });
        dayActivities.push({ id: `act-${week}-${i}-2`, type: 'Lesson', title: 'Daily Spaced Review', subtitle: 'Educational Unit', duration: '10m', questionCount: 8, skills: masteredSkills.slice(0, 2).map(s => s.name), completed: false });
      } else if (i === 2) {
        dayActivities.push({ id: `act-${week}-${i}-1`, type: 'Lesson', title: `Advanced ${getTopic(2)}`, subtitle: 'Educational Unit', duration: '15m', questionCount: 6, skills: [getTopic(2)], completed: false });
        dayActivities.push({ id: `act-${week}-${i}-2`, type: 'Guided Practice', title: `Hard-Level ${getTopic(2)}`, subtitle: 'Active Feedback', duration: '20m', questionCount: 12, skills: [getTopic(2)], completed: false });
      } else if (i === 3) {
        dayActivities.push({ id: `act-${week}-${i}-1`, type: 'Live Class', title: `${theme.title} Q&A`, subtitle: 'Scheduled Workshop', duration: '45m', skills: themeSkills.slice(0, 3).map(s => s.name), completed: false });
        dayActivities.push({ id: `act-${week}-${i}-2`, type: 'Practice Drill', title: 'Targeted Weak Spot Simulation', subtitle: 'Blind Assessment', duration: '15m', questionCount: 8, skills: learningSkills.slice(0, 1).map(s => s.name), completed: false });
      } else if (i === 4) {
        dayActivities.push({ id: `act-${week}-${i}-1`, type: 'Guided Practice', title: `Review Drill`, subtitle: 'Active Feedback', duration: '15m', questionCount: 10, skills: [getTopic(0)], completed: false });
        dayActivities.push({ id: `act-${week}-${i}-2`, type: 'Lesson', title: 'Retention Synthesis', subtitle: 'Educational Unit', duration: '10m', questionCount: 12, skills: masteredSkills.map(s => s.name), completed: false });
      } else if (i === 5) {
        dayActivities.push({ id: `act-${week}-${i}-1`, type: 'Practice Drill', title: 'Full Weekly Diagnostic', subtitle: 'Blind Assessment', duration: '35m', questionCount: 25, skills: themeSkills.slice(0, 8).map(s => s.name), completed: false });
      } else if (i === 6) {
        dayActivities.push({ id: `act-${week}-${i}-1`, type: 'Live Class', title: 'Week Reflection Recap', subtitle: 'Scheduled Workshop', duration: '20m', skills: masteredSkills.slice(0, 5).map(s => s.name), completed: false });
      }
  ...
  if (isPracticing && activeActivity) {
               ...
               <div className="text-left flex-1">
                  ...
                  <p className="text-slate-600 text-sm leading-relaxed">
                    ...
                    {correctAnswersCount === sessionHistory.length 
                      ? " Perfect accuracy! You've mastered the core procedural requirements for this week."
    ...
    const isGuidedPractice = activeActivity.type === 'Guided Practice';
        ...
        {!isDemoMode && (
          <header className="mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
            <div>
              ...
              <h1 className="text-3xl font-black text-slate-900">{WEEKS_CONFIG[selectedWeek]?.title}</h1>
            </div>
            <div className="flex items-center gap-3">
              <button 
        ...
        <div className="lg:sticky lg:top-32 space-y-6">
          <div className={`bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm transition-all duration-500 ${isDemoMode ? 'opacity-80' : ''}`}>
            ...
            <div className={`flex bg-slate-100 p-1 rounded-xl mb-6 ${isDemoMode ? 'pointer-events-none grayscale opacity-50' : ''}`}>
              <button onClick={() => setSidebarDomain(Domain.QUANT)} className={`flex-1 py-1.5 rounded-lg text-[10px] font-black transition-all ${sidebarDomain === Domain.QUANT ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>Quantitative</button>
              <button onClick={() => setSidebarDomain(Domain.VERBAL)} className={`flex-1 py-1.5 rounded-lg text-[10px] font-black transition-all ${sidebarDomain === Domain.VERBAL ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>Verbal</button>
              <button onClick={() => setSidebarDomain(Domain.DI)} className={`flex-1 py-1.5 rounded-lg text-[10px] font-black transition-all ${sidebarDomain === Domain.DI ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>Data Insights</button>
            </div>
            ...
            <div className={`flex bg-slate-100 p-1 rounded-xl mb-6 ${isDemoMode ? 'pointer-events-none grayscale opacity-50' : ''}`}>
              {[SkillStatus.MASTERED, SkillStatus.LEARNING, SkillStatus.NOT_READY].map((status) => (
                ...
                >
                  ...
                  {status === SkillStatus.NOT_READY ? 'NOT READY' : status === SkillStatus.MASTERED ? 'MASTERED' : 'LEARNING'}
                </button>
              ))}
            </div>
              ...
              {isDemoMode ? (
                ...
                <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1 custom-scrollbar">
                  {filteredSidebarSkills.length > 0 ? (
                    ...
                    <div className="py-8 text-center"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No skills in this week</p></div>
                  )}
                </div>
              )}
            </div>
